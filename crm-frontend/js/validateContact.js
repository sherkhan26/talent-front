export const validateClientContact = (contactType, contactInput) => {
    ...
    if (!contactInput.value) {
        showErrorMessage('Заполните все поля!', writeValue, contactInput)
        return false
    }
    ...
    switch (contactType.innerHTML) {
        case 'Телефон':
            if (onlyNumbers.test(contactInput.value)) {
                ...
                showErrorMessage('Номера состоят из 11 цифр!', writeValue, contactInput)
                return false
            }
        ...
        case 'Email':
            if (onlyEmail.test(contactInput.value)) {
                showErrorMessage('Неправельный Email!', writeValue, contactInput)
                return false
            }
            return true
        default:
